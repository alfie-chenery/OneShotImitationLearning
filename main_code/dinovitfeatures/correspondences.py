...
from dinovitfeatures.extractor import ViTExtractor
...
def find_correspondences(image_path1: str, image_path2: str, num_pairs: int = 10, load_size: int = 224, layer: int = 9,
    ...
    #save only the bits we need so we can remove extractor (and model) from memory
    ext_stride = extractor.stride
    ext_p = extractor.p
    del extractor
    if device == "cuda":
    ...
    for y1, x1, y2, x2 in zip(img1_y_to_show, img1_x_to_show, img2_y_to_show, img2_x_to_show):
        x1_show = (int(x1) - 1) * ext_stride[1] + ext_stride[1] + ext_p // 2
        y1_show = (int(y1) - 1) * ext_stride[0] + ext_stride[0] + ext_p // 2
        x2_show = (int(x2) - 1) * ext_stride[1] + ext_stride[1] + ext_p // 2
        y2_show = (int(y2) - 1) * ext_stride[0] + ext_stride[0] + ext_p // 2
